...
class JsonDict(dict):
    def __getattr__(self, item):
        try:
            ...
            raise AttributeError(r"'JsonDict' object has no attribute %s'" %
                                 item)
...
class Base(object):
    ...
    def _call_request(self, request_path, method, **kw):
        ...
        try:
            ret = _http_call(request_url, method, self.security, self.token,
                             **kw)
            ...
            logging.error("%s request: [%s] error [%s]" %
                          (Constants.http_protocol, request_url, ex))
            server.decr_priority()
            raise ex
